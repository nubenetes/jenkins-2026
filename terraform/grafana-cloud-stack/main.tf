...
# destroy + re-create cycle (0.1.01 create / 9.2.01 destroy), exactly like the
...
resource "grafana_cloud_stack" "this" {
  ...
  # Ephemeral by design: created by 0.1.01-grafana-cloud-bootstrap, torn down by
  # 9.2.01-grafana-cloud-decommission. No delete_protection so the decommission
