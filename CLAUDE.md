...
- [`504-BACKEND_TLS.md`](docs/504-BACKEND_TLS.md) — the **opt-in LB→pod TLS re-encryption** feature (`gateway.backendTls.enabled`, default false, override `JENKINS2026_GATEWAY_BACKEND_TLS_ENABLED`): cert-manager (pinned ArgoCD app, applied by `08.7-backend-tls.sh`) + a cluster-internal CA + per-backend server certs; the GKE `BackendTLSPolicy` (CA-validated, needs the CRD — consumers gate on `j2026_backend_tls_active`) + HTTPS `HealthCheckPolicy` generated by `09-gateway.sh`; stage-1 backend Headlamp, per-backend rollout roadmap (ArgoCD/Jenkins/pgAdmin/…)
...
- [`argocd/`](argocd/) - ArgoCD `Application`/`ApplicationSet` manifests (the GitOps
  ...
  **Argo Rollouts** (`argo-rollouts-app.yaml`, controller + Gateway API
  ...
  [`docs/501`](docs/501-PLATFORM_OPERATIONS.md) § Progressive Delivery),
  **cert-manager** (`cert-manager-app.yaml`, applied by `08.7-backend-tls.sh`
  ONLY when `gateway.backendTls.enabled` — the opt-in LB→pod TLS re-encryption,
  see [`docs/504`](docs/504-BACKEND_TLS.md)), and
