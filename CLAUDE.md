...
- **Branches**: `main` (the 9 stable root Microservices pipeline jobs are
  generated from, and deploy from, this branch), `develop` (the
  `pac-dev/*-develop` Jenkins dev-sandbox jobs - generated by
  `pac-dev/seed-jobs-dev` - check out Jenkinsfile.microservices/shared library
  from this branch; upstream Microservices itself has no `develop` branch, so
  both stable and dev-sandbox jobs build Microservices's `main`). Jenkins itself
