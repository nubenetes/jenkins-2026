...
- **`tekton-pipelines`** (wave 0) — source [`tekton/components/pipelines`](tekton/components/pipelines): a kustomization whose **remote resource** is the pinned upstream `pipeline/.../release.yaml` (Tekton has no official Helm chart). A kustomize patch points Tekton's `config-tracing` at the in-cluster OTel collector. Oversized-CRD handling (`ServerSideApply=true` + `Replace=true` + `ServerSideDiff=true`) — same rationale as `cnpg-operator`.
- **`tekton-triggers`** (wave 1) — [`tekton/components/triggers`](tekton/components/triggers): pinned `triggers/.../release.yaml` + `interceptors.yaml`.
- **`tekton-dashboard`** (wave 1) — [`tekton/components/dashboard`](tekton/components/dashboard): pinned `dashboard/.../release-full.yaml` (read-write GUI). Exposed behind Google IAP by `scripts/09-gateway.sh`.
- **`tekton-pipeline-as-code`** (wave 2) — the repo-root [`tekton/`](../tekton) dir (Tasks/Pipelines/Triggers/RBAC + the `tekton-ci` ServiceAccount), synced into the `tekton-ci` namespace. Wave 2 so the Pipelines/Triggers CRDs (waves 0–1) exist first.

Pinned versions live in `tekton/components/*/kustomization.yaml` (kept in sync with `ci.tekton.versions` in `config/config.yaml`). Credential Secrets are **script-managed** (env-sourced; created by `01-namespaces.sh`/`08.5-argocd.sh`), never owned by ArgoCD. The per-service `PipelineRun`s are kicked by `scripts/06-tekton-pipelines.sh` (run instances, not GitOps). **Teardown**: deleting the parent `Application` cascade-prunes all four children; `scripts/down.sh` does this before uninstalling ArgoCD, and switching back to `ci.engine=jenkins` removes it via `scripts/04-jenkins.sh`. **Day-2 redeploy**: [`Day2.redeploy.03-tekton`](../.github/workflows/Day2.redeploy.03-tekton.yml).
