# Argo CD Configuration (pinned to stable 3.5.x)

This directory contains manifests and configurations for deploying and managing Argo CD.

## Version policy: pinned to stable 3.5.x

ArgoCD is pinned to the latest stable **`3.5.x` line** (`v3.5.2` GA, Helm chart `10.8.4`). Both knobs live in [`config/config.yaml`](../config/config.yaml) `argocd` (`version_constraint: "3.5.x"` + `chartVersion: "10.8.4"`). The image is resolved dynamically by `scripts/08.5-argocd.sh` and continuously kept up to date by the daily `argocd-version-patch-watcher` CronJob. See [`docs/602-VERSION_PINNING.md`](../docs/602-VERSION_PINNING.md).

### Argo CD 3.5.x architectural notes & features

Upgrading to v3.5 introduces several structural improvements and changes:
...
| **`backstage`** *(only `backstage.enabled`, **default true**)* | **app-of-apps** | the `backstage-db` CNPG `Cluster` CR (wave 0) · `backstage-chart` (wave 1) | **Heterogeneous + ordered**: a raw CNPG `Cluster` must be Healthy *before* the portal chart starts; the child is **multi-source** (official `backstage/charts` 2.8.2 + `$values` → [`helm/backstage/`](../helm/backstage), with the backend-TLS overlay layered conditionally). Branch / image tag (auto-tracking the deploy branch) / `backendTls` / chart version thread down as parent helm params, sed-applied by [`08.95-backstage.sh`](../scripts/08.95-backstage.sh). See [505](../docs/505-BACKSTAGE.md). |
...
flowchart TD
    ...
    subgraph AOA["app-of-apps — curated heterogeneous groups (Helm-chart parent)"]
        ...
        PP --> PGA["pgadmin"]
        BST["Application: backstage<br/>(backstage.enabled, default true)"] --> BDB["backstage-db CNPG Cluster · wave 0"]
        BST --> BCH["backstage chart · wave 1"]
...
- **Per-cluster companion inputs stay script-managed** (not in any app, so ArgoCD never owns/prunes them): the `grafana-runtime-config` ConfigMap (`GF_SERVER_ROOT_URL`), created by [`scripts/03-observability.sh`](../scripts/03-observability.sh) and consumed via `grafana.envValueFrom` (`optional: true`). (The former `grafana-jenkins-ds` Secret was retired 2026-07-13 with the oss Jenkins datasource — Enterprise-only plugin, see docs/301.)
...
Only applied when `ci.engine=tekton` (by [`scripts/04-tekton.sh`](../scripts/04-tekton.sh), one of the four mutually-exclusive CI engines — see [`docs/404-TEKTON.md`](../docs/404-TEKTON.md)). The parent `Application` renders the local Helm chart [`tekton/`](tekton), which emits seven child `Application`s, ordered by `argocd.argoproj.io/sync-wave`:
...
Only applied when `ci.engine=githubactions` (by [`scripts/04-githubactions.sh`](../scripts/04-githubactions.sh) — see [`docs/405-GITHUB_ACTIONS.md`](../docs/405-GITHUB_ACTIONS.md)). The parent `Application` renders the local Helm chart [`githubactions/`](githubactions), which emits two child `Application`s for [Actions Runner Controller (ARC)](https://github.com/actions/actions-runner-controller), ordered by `argocd.argoproj.io/sync-wave`:
...
Only applied when `ci.engine=argoworkflows` (by [`scripts/04-argoworkflows.sh`](../scripts/04-argoworkflows.sh) — see [`docs/406-ARGO_WORKFLOWS.md`](../docs/406-ARGO_WORKFLOWS.md)). The parent `Application` renders the local Helm chart [`argoworkflows/`](argoworkflows), which emits three child `Application`s, ordered by `argocd.argoproj.io/sync-wave`:
