...
- **`tekton-pruner`** (wave 1) — [`tekton/components/pruner`](tekton/components/pruner): vendored `release.yaml` (v0.4.0) + a patch lowering `historyLimit` from 100 to 5 (deliberately lower than the Jenkins `buildDiscarder` 20, to keep pod/image accumulation off the 50 GiB node disk). GC of completed runs.
