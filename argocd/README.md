...
Only applied when `ci.engine=tekton` (by [`scripts/04-tekton.sh`](../scripts/04-tekton.sh), the alternative CI engine to Jenkins — see [`docs/403-TEKTON.md`](../docs/403-TEKTON.md)). The parent `Application` renders the local Helm chart [`tekton/`](tekton), which emits four child `Application`s, ordered by `argocd.argoproj.io/sync-wave`:
