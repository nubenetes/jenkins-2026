...
- **`tekton-pipelines`** (wave 0) — source [`tekton/components/pipelines`](tekton/components/pipelines): a kustomization whose **remote resource** is the pinned upstream `pipeline/.../release.yaml` (Tekton has no official Helm chart). Oversized-CRD handling (`ServerSideApply=true` + `Replace=true` + `ServerSideDiff=true`) — same rationale as `cnpg-operator`.
