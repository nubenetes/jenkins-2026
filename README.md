...
| **Grafana log severity** | `observability.logMinSeverity` | **`info`** | `trace` (keep all) · `debug` · `warn` · `error` — minimum severity shipped to the backend, applied as a `filter` in the `otel-collector-logs` DaemonSet, so it trims **every** Grafana logs panel (microservices + platform) in all four obs modes. Matches structured-line level tokens (JSON `"level"`, logfmt `level=`); plain-text lines are never dropped. Per-run override `JENKINS2026_LOG_MIN_SEVERITY`; GHA `log_min_severity` dropdown on `Day1.cluster.01-gke`. See [docs/301 § Log Levels](docs/301-OBSERVABILITY.md#log-levels). |
