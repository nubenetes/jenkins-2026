...
| **Lean metrics** | `observability.leanMetrics` | **`false`** | **`true`** *(grafana-cloud)* — drop the k8s-monitoring **cluster infra metrics** (cadvisor/kube-state/node-exporter) the custom dashboards don't use, to stay under the Grafana Cloud **free-tier 15k active-series** cap (e.g. while validating the `develop` tier). App/CNPG/Tekton metrics (via the otel-collector) are unaffected; loses the built-in "K8s Compute" views. |
