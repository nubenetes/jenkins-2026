...
> **Flag-parity invariant (guarded).** Those agent env values are resolved from the `jenkins-credentials` Secret at controller **startup** and baked into the node properties, so a value that changes in the Secret without rolling the controller leaves the running agent on the **stale** value. `04-jenkins.sh` rolls the controller via `banner_links_checksum` (a hash of exactly those config-derived values, passed as `controller.podAnnotations.bannerLinksChecksum`) — but *only for the values it lists*. Binary Authorization was flipped off, the Secret updated to `false`, yet `binauthz-enabled` was **absent** from that checksum, so the controller never rolled and the agent kept `BINAUTHZ_ENABLED=true`, failing every build at sign-and-attest against a torn-down attestor (2026-07-16). [`scripts/verify-jenkins-flag-parity.py`](../scripts/verify-jenkins-flag-parity.py) now **fails the PR** if any config-derived agent value is missing from the checksum, so this cannot recur; credentials (git/registry/OIDC tokens) are out of scope — they are provisioned separately and rotating them is a different operation.
