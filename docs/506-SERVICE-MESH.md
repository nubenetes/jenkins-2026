...
| **Pin the OTel auto-instrumentation container** | The OTel Operator instruments the **first** container when `container-names` is unset — and CSM injects `istio-proxy` **ahead** of the app, so the `-javaagent` lands on **Envoy** (a Go binary, where it is inert) and the Java app silently loses **all** instrumentation → every OTel-fed Grafana panel goes **"No data"**. Worse, the pipeline's *OTel Self-Heal* stage still reports OK: it greps *any* container in the pod for `-javaagent`, and the sidecar has one | `instrumentation.opentelemetry.io/container-names: "<app>"` pod annotation in the app chart (`gitops-config`; **self-gating** — with no mesh the app is the first container anyway) | ✅ done |
