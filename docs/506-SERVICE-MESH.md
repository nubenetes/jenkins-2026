...
| **Pin the OTel auto-instrumentation container** | The OTel Operator instruments the **first** container when `container-names` is unset — and CSM injects `istio-proxy` **ahead** of the app, so the `-javaagent` lands on **Envoy** (a Go binary, where it is inert) and the Java app silently loses **all** instrumentation → every OTel-fed Grafana panel goes **"No data"**. Worse, the pipeline's *OTel Self-Heal* stage reported OK anyway: it grepped *any* container in the pod for `-javaagent`, and the sidecar had one — **since hardened** ([`microservicesOtelSelfHeal`](../vars/microservicesOtelSelfHeal.groovy) + the Tekton/Argo twins) to check the **app container by name** and flag a mis-target as **UNSTABLE** instead of green | `instrumentation.opentelemetry.io/container-names: "<app>"` pod annotation in the app chart (`gitops-config`; **self-gating** — with no mesh the app is the first container anyway) | ✅ done |
| **Let in-cluster non-mesh callers in** | The pipeline's **Smoke Test** curls each service's health endpoint from a throwaway pod in the **`jenkins`** namespace, which is *not* meshed → STRICT resets the connection (**`curl exit 56` — "Failure receiving network data"**) and fails the build. Same class as the ingress-edge rule, different caller: anything in-cluster that talks to a meshed pod without being a mesh client | [`08.85`](../scripts/08.85-service-mesh.sh) adds a per-service workload `PeerAuthentication` (`<svc>-nonmesh-permissive`) keeping **only the app's own HTTP port** PERMISSIVE (the gateway has its own rule). The east-west gateway→backend hop still negotiates identity mTLS between the two sidecars — PERMISSIVE *accepts* mTLS, it just doesn't *require* it on that port | ✅ done |
