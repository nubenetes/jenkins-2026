...
- **Ephemeral** (simplest for a PoC): `count`-gated, destroyed on `Decom` and
  ...
  **But "destroyed" is a lie for KMS, and that bit us live** ([run 29433092250](https://github.com/nubenetes/jenkins-2026/actions/runs/29433092250)): a KMS **keyring can never be deleted** and a **key only enters a scheduled-destruction window** — so `terraform destroy` removes them from *state* while they **linger in GCP**, and the next `Day1`'s create then **409s** (`KeyRing … already exists`). Stable names + no `prevent_destroy` are necessary but **not sufficient**: the crypto-key re-adopts its scheduled-destroyed version by name, but the keyring create still collides. The `Day1.cluster.01` **"Terraform apply"** step itself now `terraform import`s the survivors into state **only when they already exist** (an `import {}` block can't — it fails `plan` on a truly-fresh project where the keyring doesn't exist yet) — inline in that step, not a standalone one, because a first attempt at a separate step failed **invisibly**: it never exported `TF_VAR_project_id` (only a plain shell var for `gcloud`), so the import itself errored with "No value for required variable", and a naive `\|\| true` swallowed that too (run 29434750534) — `apply` then hit the same 409 with nothing actually imported. Doing it inline guarantees the import sees the exact same `TF_VAR_*` the apply itself needs, and each import result is captured (`if VAR=$(cmd); then …`, safe under the step's `set -e`) so a genuine failure surfaces as `::warning::` instead of vanishing — only "already managed" (an idempotent re-run) is treated as benign. **Importing the crypto-key object is not the whole story**, either: its *version* only enters a scheduled-destruction window (up to 30 days), so the object imports cleanly while its sole version sits `DESTROY_SCHEDULED` — and `apply`'s `data.google_kms_crypto_key_version` (which reads the primary version's public key for the attestor's trust) then 400s: `... is not enabled, current state is: DESTROY_SCHEDULED`. The same step **restores + re-enables** any such version (`gcloud kms keys versions restore` → `enable`) — the same key material the attestor already trusts, not a freshly-minted one, and reversible. The `google_gke_hub_membership` (mesh) has a sibling create-time hazard — the Fleet API rejects it while the cluster is mid-operation (`Error code 9`) — fixed by depending on the node pool + a create timeout so Terraform waits for the cluster to go idle.
