...
- **Pushes to the forks never create PaC PipelineRuns — GitHub shows the delivery `202 OK` but the PaC controller logs `webhook-secret=NOTFOUND` → `could not validate payload … no webhook secret has been set, in repository CR or secret`**: the `pac-webhook` Secret's `webhook.secret` key is **empty** (PaC refuses HMAC validation outright on an empty secret — correct security posture, but the whole git-push path dies silently: runs simply never appear). Root cause (found live 2026-07-12): `PAC_WEBHOOK_SECRET` is an *optional* GitHub secret most workflows don't pass, and `01-namespaces.sh`'s imperative branch used to write `"${PAC_WEBHOOK_SECRET:-}"` **unconditionally** — so any `01`-running workflow (e.g. a `Day2.redeploy.08-backstage`) clobbered back to `""` the real HMAC that `06-tekton-pipelines.sh`'s `ensure_pac_webhook_secret` had generated and stamped on the fork webhooks at Day1. **Fixed twice over**: `01` is now keep-if-present + generate-if-absent for all three webhook HMACs (`pac-webhook`, `tekton-github-webhook-secret`, `argoworkflows-github-webhook`), and both `06` scripts now **re-assert (PATCH) the current HMAC onto an already-existing fork hook** instead of trusting it (reconcile-to-current, [docs/104](./104-REBUILD_SAFETY.md) — the fork hook persists across cluster rebuilds while the in-cluster secret regenerates, so a mismatch was otherwise guaranteed). Live remediation on an already-broken cluster: re-run `06-tekton-pipelines.sh` (or `Day2.redeploy.03-tekton`), which heals both ends; then redeliver the failed delivery from the fork's *Settings → Webhooks → Recent Deliveries* (GitHub re-signs redeliveries with the current hook secret).
