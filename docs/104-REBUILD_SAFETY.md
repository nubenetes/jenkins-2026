...
| **Never-prune-a-live-reference** | Retention deletes only *untagged* manifests, never a tag the GitOps repo might pin — **microservices** (`gateway`/`jhipstersamplemicroservice`) only; **Backstage** never goes untagged (`backstage.image.tag` tracks the deploy branch, a moving tag, not a frozen GitOps pin — [505](./505-BACKSTAGE.md#scorecard-tab-entity-kpis)), so its job instead keeps-by-count and protects whatever `main`/`develop` currently *resolve to* by digest, resolved fresh each run (`ignore-versions` on a container regex-matches the manifest **digest**, not any tag — a confirmed upstream limitation, [actions/delete-package-versions#88](https://github.com/actions/delete-package-versions/issues/88)) | ghcr retention ([#488](https://github.com/nubenetes/jenkins-2026/pull/488), backstage split found live 2026-07-13) |
...
| Resource | Rebuild-safe? | Mechanism |
|---|---|---|
| **CSM Fleet membership** (`google_gke_hub_membership`) | ✅ *(after the fix below)* | Persistent, fixed-identity (registered by a stable name), but **Terraform `count`-gated on `serviceMesh.mode` and keyed to the cluster's own identity** → `Decom` (`terraform destroy`) removes it and `Day1` re-registers it **in lockstep with the cluster** (*reconcile-to-current*). A no-op when `mode=none`. **Found live** ([run 29381924836](https://github.com/nubenetes/jenkins-2026/actions/runs/29381924836)): that removal is **conditional on the cluster being idle** — the Fleet API rejects a membership delete outright while any cluster operation is in flight (`Error code 9 … cluster is currently running another operation. Please retry after the operation is done`), and `down.sh` has just been deleting namespaces + NEGs, so one usually still is. `Decom.cluster.01` now **waits for in-flight GKE operations to drain before `terraform destroy`** (*cluster-alive teardown ordering* — the same wait `Day2.scale.01-pause` does before resizing) and retries the destroy once. Left unfixed this is a **mode-B residue** hazard: a membership surviving a red teardown would collide with the next `Day1`'s re-registration. See [506 § Rebuild-safety](./506-SERVICE-MESH.md#rebuild-safety). |
| **CSM health-check firewall rule** (`gke-csm-thc-*`) | ✅ *(after the fix below)* | **Controller-created, never Terraform-owned**: Cloud Service Mesh adds it to the VPC and **GKE does not remove it with the cluster**, so it **pins the network** and the `Decom` VPC delete fails — `The network resource '…' is already being used by … firewalls/gke-csm-thc-…` ([run 29392065023](https://github.com/nubenetes/jenkins-2026/actions/runs/29392065023)). The twist: it is only an **orphan once the cluster is gone** — i.e. *mid-`terraform destroy`*, long after `down.sh`'s cluster-alive NEG/forwarding-rule sweep has finished — so no pre-destroy sweep can catch it. `Decom.cluster.01` therefore sweeps it in the **destroy's failure path** — **parsing the blocker's name straight out of GCP's own error** (`… used by …/global/firewalls/<name>`), so it deletes only what GCP just named as pinning *this* network, then retries. Same *cluster-alive teardown ordering* idea as `down.sh`, extended **past the cluster's own death**. ⚠️ The first attempt at this looked the VPC up via a **new `terraform output`** and silently no-op'd: **`terraform output` reads the *state*, and a freshly-added output is absent from a state written by an older `apply`** — a `destroy` never adds one. Don't reach for an output you only just declared during a teardown. |
| **CSM Fleet membership** — *create* side (`google_gke_hub_membership`) | ✅ *(after the fix below)* | Symmetric to the teardown hazard above: on a from-zero `Day1` the Fleet API rejects the membership **create** while the cluster is still mid-operation (`Error code 9 … cluster is currently running another operation`, [run 29433092250](https://github.com/nubenetes/jenkins-2026/actions/runs/29433092250)). `depends_on = [google_container_node_pool.primary]` (the last thing to settle) makes Terraform wait for the cluster+pool create to finish before touching Fleet, plus a create `timeouts`. |
| **Binary Authorization** KMS keyring/key + attestor + Container Analysis note | ✅ *(after the fix below)* | Persistent, fixed-identity, and **KMS never hard-deletes** — a keyring **cannot** be deleted, a key only enters a **scheduled-destruction window** — so `Decom`'s `terraform destroy` drops them from **state** while they **linger in GCP**, and the next `Day1` create **409s** (`KeyRing … already exists`, [run 29433092250](https://github.com/nubenetes/jenkins-2026/actions/runs/29433092250)) — the canonical fixed-identity **collision** ([§2](#2-the-bug-class--two-failure-modes)). Stable names + no `prevent_destroy` are necessary but **not sufficient** (the key re-adopts its scheduled version by name; the **keyring** create still collides). The **"Terraform apply"** step's own script `terraform import`s the survivors into state **only when they already exist** — *tombstone-adoption*, done inline in that step (not a standalone one) because a standalone attempt failed invisibly on a missing `TF_VAR_project_id`, swallowed by a naive `\|\| true` (run 29434750534); an `import {}` block would separately fail `plan` on a genuinely-fresh project. See [507 § Rebuild-safety](./507-BINARY-AUTHORIZATION.md#rebuild-safety). |
