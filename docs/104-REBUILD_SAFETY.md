...
| **Binary Authorization** KMS keyring/key + attestor + Container Analysis note | ✅ *(after the fix below)* | Persistent, fixed-identity, and **KMS never hard-deletes** — a keyring **cannot** be deleted, a key only enters a **scheduled-destruction window** — so `Decom`'s `terraform destroy` drops them from **state** while they **linger in GCP**, and the next `Day1` create **409s** (`KeyRing … already exists`, [run 29433092250](https://github.com/nubenetes/jenkins-2026/actions/runs/29433092250)) — the canonical fixed-identity **collision** ([§2](#2-the-bug-class--two-failure-modes)). Stable names + no `prevent_destroy` are necessary but **not sufficient** (the key re-adopts its scheduled version by name; the **keyring** create still collides). The Day1 workflow's **"Adopt lingering KMS keyring/key"** step `terraform import`s the survivors into state **only when they already exist** — *tombstone-adoption* done in the workflow because an `import {}` block would fail `plan` on a genuinely-fresh project. See [507 § Rebuild-safety](./507-BINARY-AUTHORIZATION.md#rebuild-safety). |
