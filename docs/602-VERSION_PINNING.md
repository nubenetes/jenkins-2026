...
| **Backstage** chart (official `backstage/charts`) | `2.8.2` | `config.yaml` `backstage.chart.version` | ArgoCD `targetRevision` on the app-of-apps child — the pin is threaded down as the parent's `chart.version` helm param by [`08.95-backstage.sh`](../scripts/08.95-backstage.sh) ([505](./505-BACKSTAGE.md)) |
| **Backstage app** (core + plugins, baked into the custom image) | core `1.52.1` (release-manifest **exact** pins) · jenkins `0.32.0`/`-backend 0.29.0` · github-actions `1.2.0` · tekton `3.39.0` · argocd `2.9.0`/`-backend 1.5.0` · gcp-iap `0.4.16` · techdocs-mermaid `0.26.0` (all verified 2026-07-11) | [`backstage/package.json`](../backstage/package.json) (+ `backstage.json`) | npm exact pins compiled into the ghcr image by `Day2.publish.06-backstage`; the deployed **image tag auto-tracks the deploy branch** (`main`/`develop`, moving), with `sha-<sha>` tags for pinning via `backstage.image.tag` / `JENKINS2026_BACKSTAGE_IMAGE_TAG` |
| **Backstage image toolchain** | Node `24` (`node:24-trixie-slim`) · yarn `4.8.1` (`packageManager`) · mkdocs-techdocs-core `1.7.0` | [`backstage/packages/backend/Dockerfile`](../backstage/packages/backend/Dockerfile) + [`backstage/package.json`](../backstage/package.json) | base-image tag + corepack + pip `==` pin |
...
| **CI agent images** (shared across three engines — Jenkins pod templates · Tekton tasks · Argo Workflows; the **ARC engine builds runner-native** instead: `actions/setup-java` Temurin 21 for JDK parity, `docker run` semgrep/trivy, CodeQL via `github/codeql-action` — see [405. GitHub Actions](./405-GITHUB_ACTIONS.md) and Known residuals) | maven `3.9.9`, node `20-bookworm`, docker `26-dind`, alpine/k8s `1.31.3`, semgrep `1.79.0`, trivy `0.52.2`, k6 `2.0.0`, `alpine/git:2.54.0`, `codeql-container@c6f3f8cb…` | [`vars/MicroservicesPipeline.groovy`](../vars/MicroservicesPipeline.groovy) · [`tekton/tasks/*.yaml`](../tekton/tasks/) · [`argoworkflows/`](../argoworkflows/) | image tag/digest (the three engines share the same git + codeql pins) |
