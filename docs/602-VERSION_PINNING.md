...
| **Argo Workflows + Argo Events** (vendored release YAMLs — **bump both together**) | `v3.6.4` (workflows) / `v1.9.4` (events) | `config.yaml` `argoworkflows.versions.{workflows,events}` + **vendored** [`argocd/argoworkflows/components/`](../argocd/argoworkflows/) | pinned release YAML, applied by `04-argoworkflows.sh` when `ci.engine=argoworkflows` |
