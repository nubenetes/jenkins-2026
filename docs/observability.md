...
- **`k6-smoke-overview.json`** (uid `jenkins2026-k6-smoke-overview`) - the
  `petclinic-k6-smoke` / `pac-dev/petclinic-k6-smoke-develop` run's own
  iterations, checks pass rate, `http_req_failed` rate and
  `http_req_duration` p95 (filterable by `stable`/`develop`, with thresholds
  matching `jenkins/pipelines/k6/petclinic-smoke.js`), plus the traces and
  PetClinic pod logs generated by that run. Linked directly from the build
  console - see [k6 observability smoke test](#k6-observability-smoke-test)
  below.

## k6 observability smoke test

[`petclinic-k6-smoke`](../jenkins/pipelines/Jenkinsfile.petclinic-k6-smoke) /
`pac-dev/petclinic-k6-smoke-develop` (generated by `seed_jobs.groovy`
alongside the per-service pipelines, see
[`docs/pipelines-as-code.md`](pipelines-as-code.md#k6-observability-smoke-test-petclinic-k6-smoke))
...
default 12) - through every PetClinic Service in `TARGET_NAMESPACE`
(`petclinic`/`petclinic-develop`). This is **not** a load/stress test; the
...
runs one simulated "owner browses PetClinic" session per iteration:
...
- Direct `/actuator/health` checks against `config-server`,
  ...
  `petclinic.genaiServiceEnabled` in `config/config.yaml` - skipped by
  default since its image/pod may not be present without a real
  `OPENAI_API_KEY`).
- `petclinic-angular`'s `/` (SPA shell).
...
above), every PetClinic service the iteration touches continues that
trace - so one k6 iteration produces one Tempo trace spanning (a subset
of) all 9 services, with each service's logs/metrics correlated to it the
...
[`vars/petclinicK6Smoke.groovy`](../vars/petclinicK6Smoke.groovy) also runs
...
`service.namespace=jenkins-2026,deployment.environment=<stable|develop>` -
the same resource attributes as the PetClinic services - so the test run's
...
`petclinicK6Smoke` prints to the build console:
...
dashboard link in `petclinicK6Smoke`'s build output. Two ways to provide it:
