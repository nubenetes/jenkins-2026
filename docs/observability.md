...
per namespace (`microservices`/`microservices-develop`), pointing at
...
- **`k6-smoke-overview.json`** (uid `jenkins2026-k6-smoke-overview`) - the
  `microservices-k6-smoke` / `pac-dev/microservices-k6-smoke-develop` run's own
...
[`microservices-k6-smoke`](../jenkins/pipelines/Jenkinsfile.microservices-k6-smoke) /
`pac-dev/microservices-k6-smoke-develop` (generated by `seed_jobs.groovy`
...
(`microservices`/`microservices-develop`). This is **not** a load/stress test; the
