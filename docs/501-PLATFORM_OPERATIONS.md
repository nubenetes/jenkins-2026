[← Previous: 406. Argo Workflows](./406-ARGO_WORKFLOWS.md) | [🏠 Home](../README.md) | [→ Next: 502. Microservices GitOps](./502-MICROSERVICES_GITOPS.md)
...
Flip per engine with the config flag (durable) or the **`run_node_pool` input** on that engine's redeploy workflow — `Day2.redeploy.02-jenkins` / `Day2.redeploy.03-tekton` / `Day2.redeploy.06-githubactions` / `Day2.redeploy.07-argoworkflows` (per-run, no commit). See the Tekton-specific mechanics and the RWX/affinity-assistant alternatives in [`docs/404`](404-TEKTON.md) (and the ARC / Argo Workflows equivalents in [`docs/405`](405-GITHUB_ACTIONS.md) / [`docs/406`](406-ARGO_WORKFLOWS.md)).

#### Scoping the standard DaemonSets off the tiny default-NAP overflow nodes

Cluster-level NAP backs the `ci-spot` ComputeClass, but it is also a **general cluster autoscaler**: for **any** non-ComputeClass pod that can't fit the static pool — most often a *platform* pod that briefly overflows during a build burst — it auto-provisions an **untainted overflow node**, picking the **smallest machine that fits** (NAP has no minimum-size lever). That node can be an **`e2-medium` (940m allocatable CPU)**, ~98% of which GKE's own system DaemonSets already consume. A DaemonSet that targets *every* node then has **nowhere to schedule** on that overflow node: its pod sits **`Unschedulable` / `FailedScheduling: Insufficient cpu`** and is flagged red in the Cloud Console workload view — even though the platform is healthy. This is the **only** path in the cluster that produces tiny nodes; `ci-spot` never does (its `priorities` floor every rule at `minCores: 4`).

Two DaemonSets hit exactly this on a live cluster and were fixed by **scoping them with a `nodeAffinity`** to the pools where their work belongs, so they never chase the overflow node.

**The three node types this cluster runs — and which can host the standard DaemonSets:**

| Node type | Purpose | Machine sizing | Taint & labels | What runs there | Hosts the standard DaemonSets? |
|---|---|---|---|---|---|
| **`jenkins-2026-pool`** (static; `= ${cluster_name}-pool`, [`terraform/gke`](../terraform/gke/main.tf)) | Long-lived **platform** + build agents when `runNodePool=static` (the default) | Fixed **`e2-standard-8`** (8 vCPU) | **untainted**; label `cloud.google.com/gke-nodepool=jenkins-2026-pool` | ArgoCD · the active CI engine · observability · CNPG · build agents (default) | ✅ **yes** — 8 vCPU, ample headroom |
| **`ci-spot`** Custom ComputeClass ([`infrastructure/compute-classes/ci-spot.yaml`](../infrastructure/compute-classes/ci-spot.yaml)) | Elastic **Spot**, scale-to-zero CI build agents when `runNodePool=ci-spot` | NAP-sized, **floored at `minCores: 4` / `minMemoryGb: 16`** — e2-micro/small/medium excluded by design | taints `cloud.google.com/compute-class=ci-spot:NoSchedule` (+ `gke-spot=true:NoSchedule`); label `cloud.google.com/compute-class=ci-spot` | CI build agents only (they carry the matching nodeSelector + tolerations) | ✅ yes (≥4 vCPU) — **but** a DaemonSet must *tolerate* the taints to land here |
| **default-NAP overflow** (a `nap-<type>-*` pool) | Whatever **non-ComputeClass** pod overflowed the static pool (usually a platform pod) | **smallest machine that fits** the pending pod + system DaemonSets — e.g. **`e2-medium` (940m)** | **untainted** (so unscoped pods *and* every-node DaemonSets are eligible) | the one overflow pod + GKE system DaemonSets (~98% of an `e2-medium`) | ❌ **no** on a tiny pick — no CPU left for a 50m node-exporter → `Unschedulable` |

**The two DaemonSets, the bug, and the scope applied:**

| DaemonSet | What it does | The bug (ran on **every** node) | Scope applied (`nodeAffinity`, `requiredDuringScheduling…`) | Why it's safe to scope | Commit |
|---|---|---|---|---|---|
| **`agent-image-prepull`** ([helm/jenkins](../helm/jenkins/agent-image-prepull.yaml) · [tekton](../tekton/agent-image-prepull.yaml) · [argoworkflows](../argoworkflows/agent-image-prepull.yaml)) | Warms the build-agent image cache — pre-pulls the ~9 multi-GB pipeline images (maven/node/dind/k8s/git/semgrep/trivy/k6/codeql) so build pods start fast | its prepull pod on the `e2-medium` overflow node sat `Unschedulable` (`Insufficient cpu`) | `cloud.google.com/gke-nodepool In [jenkins-2026-pool]` — **static pool only** | **Lossless**: agents run on the static pool by default (`runNodePool=static`), and the prepull never *tolerated* the `ci-spot` taint, so it never warmed Spot nodes anyway | [`cfc95fc`](https://github.com/nubenetes/jenkins-2026/commit/cfc95fc) |
| **managed-\*** `prometheus-node-exporter` ([`03-observability.sh`](../scripts/03-observability.sh)) | Feeds node host-metrics to the collector → Azure Monitor / Amazon Managed Prometheus; installed **only** in `observability.mode=managed-azure` / `managed-aws` | same — its pod on the overflow node sat `Unschedulable` (`Insufficient cpu`) | **two** `nodeSelectorTerms` (an OR): `gke-nodepool In [jenkins-2026-pool]` **OR** `compute-class In [ci-spot]` — static pool **+** Spot CI nodes | **Acceptable, not lossless**: the persistent platform + Spot-CI nodes (whose host-metrics matter) keep node-exporter; only the minutes-lived overflow nodes lose it. The NAP/Spot dashboard reads KSM `kube_node_*` (a Deployment), **not** node-exporter, so it is unaffected | [`f9f25a2`](https://github.com/nubenetes/jenkins-2026/commit/f9f25a2) |

**Problem → cause → solution** — the "Unschedulable DaemonSet on a tiny NAP overflow node" class:

| | |
|---|---|
| **Symptom** | Cloud Console flags a DaemonSet pod `Unschedulable`; `kubectl describe pod` shows `FailedScheduling: Insufficient cpu`; the node is a small `nap-e2-*` (e.g. `e2-medium`). The platform itself is healthy. |
| **Root cause** | Default NAP provisioned an **untainted, smallest-that-fits** overflow node for a platform pod; it is too small (system DaemonSets eat ~98%) for a standard DaemonSet that targets every node. |
| **Why not fix it at the NAP level?** | You can't, cleanly — see the three bullets below. |
| **Solution** | Scope the DaemonSet with a `nodeAffinity` to `gke-nodepool=jenkins-2026-pool` (+ `compute-class=ci-spot` where its host-metrics matter), as `agent-image-prepull` and the managed-\* `prometheus-node-exporter` now do. |

**Why a true "constrain NAP so it never makes tiny nodes" fix isn't cleanly available in GKE** — the honest design rationale, so that scoping the DaemonSets *is* the fix, not a workaround:

- **NAP has no machine-family or minimum-size field.** GKE NAP's `auto_provisioning_defaults` (the Terraform `google`-provider `cluster_autoscaling` block in [`terraform/gke/main.tf`](../terraform/gke/main.tf)) exposes disk size, node SA, image type, Shielded/auto-repair/upgrade — but **no** minimum machine size or family allow-list. NAP always picks the *smallest node that fits* the pending pod + DaemonSets, so it can and will pick an `e2-medium`.
- **Cluster-level NAP can't be disabled.** The `ci-spot` ComputeClass's `nodePoolAutoCreation` **requires** cluster NAP to be on; turning NAP off to stop the tiny nodes would break `ci-spot` (the whole Spot-CI elasticity). So NAP stays on and keeps making overflow nodes.
- **Therefore scope the two "chase-every-node" DaemonSets** — the effective, surgical fix, rather than a NAP-level constraint that GKE does not offer.

> **Deeper trigger (worth a note).** The platform overflows onto tiny nodes at all because the autoscaler runs the **`OPTIMIZE_UTILIZATION`** profile, which prefers spinning up a cheap small node over scaling the `e2-standard-8` static pool. Those overflow nodes then **persist** while they hold packed platform pods (they sit ~95% full, so the autoscaler won't consolidate them) — which is why a stray `e2-medium` lingers long enough to be flagged in the Console. Right-sizing a *new* every-node DaemonSet, or scoping it like these two, is the durable habit — see the [checklist in 902](./902-TROUBLESHOOTING.md#a-daemonset-pod-is-unschedulable-insufficient-cpu-on-a-small-nap-overflow-node).
...
* **Edge TLS (GKE Gateway API)**: TLS terminates at the global L7 LB (the Google-managed wildcard cert). **By default**, the LB→pod hop is **plain HTTP** — that hop never leaves Google's VPC fabric and rides Google's default network-layer encryption in transit; pod-to-pod traffic is covered by the WireGuard bullet below. **Opt-in hardening**: `gateway.backendTls.enabled` (default `false`) adds application-layer TLS + server authentication on that same hop for six converted backends (Headlamp, faro, ArgoCD, pgAdmin, OSS Grafana, Jenkins) via an in-cluster PKI (cert-manager) + a GKE `BackendTLSPolicy` — see **[504. Backend TLS](./504-BACKEND_TLS.md)** for the full mechanics, the per-backend roadmap, and a decision guide on whether you actually need it for your deployment.
...
* **Agent image pre-pull DaemonSet**: a per-engine `agent-image-prepull` DaemonSet ([helm/jenkins](../helm/jenkins/agent-image-prepull.yaml) · [tekton](../tekton/agent-image-prepull.yaml) · [argoworkflows](../argoworkflows/agent-image-prepull.yaml), applied by the matching `04-<engine>.sh`) warms the node image cache by pre-pulling the ~9 multi-GB pipeline images (maven/node/dind/k8s/git/semgrep/trivy/k6/codeql) out-of-band via `initContainers` that pull-and-exit — so a build pod isn't blocked on a cold multi-image pull (a pod only goes `Running` once **all** its container images are present). It is **`nodeAffinity`-scoped to the static `jenkins-2026-pool`** — where agents run by default and where the platform lives — so it never chases the tiny default-NAP overflow nodes (where its pod would sit `Unschedulable`); and since it does not tolerate the `ci-spot` taint it does not warm Spot nodes, so the first build on a freshly auto-provisioned Spot node pays a cold pull (see the [NAP → Spot runbook § cold-start](./runbooks/nap-spot-provisioning.md)). Full scheduling rationale + the node-type/DaemonSet-scoping matrices: [§ 2 · Scoping the standard DaemonSets](#scoping-the-standard-daemonsets-off-the-tiny-default-nap-overflow-nodes).
...
| ArgoCD | `https://argocd.<baseDomain>` | **yes** (IAP + Dex `authproxy` SSO — IAP does the login, ArgoCD trusts the identity header for per-user RBAC) |
...
| **ArgoCD** | **Yes** (`GCPBackendPolicy` IAP on `argocd-server`, like the other UIs) | **Dex `authproxy`** connector: IAP does the one Google login at the edge + injects `X-Goog-Authenticated-User-Email`; ArgoCD trusts that header (single sign-on, no second login). Break-glass = local `admin` via `argocd login --core`. | ArgoCD RBAC by email, **prefixed `accounts.google.com:`** (IAP header format): default `role:readonly`, admin email = `role:admin`. ⚠️ authproxy trusts the header un-verified → the `argocd-baseline` NetworkPolicy limits `:8080` ingress to the LB + CI namespaces (header-spoof). CI deploy callers are unaffected (internal API-token auth). |
