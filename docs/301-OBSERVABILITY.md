...
| **CNPG / cAdvisor / kube-state _counter_ panels empty on managed/cloud but fine on OSS** | OSS scrapes those exporters directly → keeps the source name; in the modes where the **collector self-scrapes and remote-writes via OTLP** (grafana-cloud/azure/aws), the OTLP→managed-Prometheus path **appends `_total`** to every monotonic counter. So bare `cnpg_pg_stat_*` → `*_total`, and cAdvisor's already-`_total` `container_cpu_usage_seconds_total` → `container_cpu_usage_seconds_total_total` (double). A dashboard querying the source name finds nothing | Union both names with **`or`** using **exact** names (not a `__name__` regex — see the Azure row): `rate(NAME{…}[…]) or rate(NAME_total{…}[…])`. Backend-agnostic (the non-existent name is empty in each backend). Applied to `postgres-overview` + `microservices-overview` Container CPU / Pod restarts |
