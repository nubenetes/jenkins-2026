...
| **`Container CPU` / `Container memory` empty on `managed-azure`/`managed-aws` even though the metric exists** | The OTLP round-trip **drops the `container` label** from the cAdvisor series (the empty-valued `container=""` doesn't survive), so the standard `container!="", container!="POD"` filter matches nothing. `namespace`, `pod`, **`image`**, `id`, `name` all survive. (OSS/grafana-cloud scrape cAdvisor directly → `container` is present, which is why the filter works there.) | Filter with **`image!=""`** instead — the portable idiom the bundled community *k8s-views* boards use: `image` survives on every backend and equally excludes the pod-level cgroup total (empty `image`), so no double-count. Applied to Container CPU + memory across all variants — one canonical query, all backends |
