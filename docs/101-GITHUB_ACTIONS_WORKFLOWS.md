...
| S5 | `tls=none`, binauthz=**on** | isolates binauthz from the mesh | attestation created, no mesh objects | ✅ live-validated 2026-07-16/17 (covered by S7–S9, every one of which runs `tls=none` + binauthz + a real build that attests) |
| S5b | `tls=backend-tls`, binauthz=**on** | the **third** TLS state composes with binauthz too (S4 proved it for the mesh — the harder case where the sidecar could touch egress; this proves the LB→pod hop is likewise orthogonal) | backend-TLS stack up **and** an attestation created | ✅ **live-validated 2026-07-17** (Day1 `jenkins`+`backend-tls`+binauthz from `main`): BackendTLSPolicy + HTTPS HealthCheckPolicy + cert-manager certs on all 8 backends, and two Jenkins builds `Finished: SUCCESS` **signed under backend-TLS** → new attestations `gateway@sha256:f358…` + `jhipstersamplemicroservice@sha256:acfd…`. So all three `intra_cluster_tls` values are now attest-validated with binauthz on. |
| S6 | mesh **and** backendTls both on | the **exclusion** must fire | `lib/config.sh` **fails fast** before any apply | ✅ gate live-tested |
| **S7** | binauthz=on, `ci.engine` = **tekton** | signing is per-engine | attestation created by Tekton | ✅ **live-validated 2026-07-16** (attestation for `jhipstersamplemicroservice@sha256:3007cd…` by `jenkins-2026-attestor`). Enablement = three parts, all now in place: **digest** (Jib's `target/jib-image.digest` → `image@sha256`), **self-gate** (`BINAUTHZ_ENABLED` checked before the script, so a binauthz-off build doesn't 127 on infra cloned from a branch lacking the script), and **flag+KSA** — the flag threads as a Tekton param (run→pipeline→task, rendered by `06-tekton`), and `04-tekton` annotates the `tekton-ci` KSA to impersonate the signer GSA. **Found live**: a plain `kubectl annotate` on that ArgoCD-managed SA was **reverted by selfHeal before the sign step ran** — fixed with `ignoreDifferences` on the `tekton-pipeline-as-code` app for that one annotation. Exactly the class a guess would have shipped as a silent false-green. |
| **S8** | binauthz=on, `ci.engine` = **githubactions** | signing is per-engine | attestation by GHA | ✅ **live-validated 2026-07-17** (`gateway@sha256:9b70…` + `jhipster@sha256:1eb2…` by `jenkins-2026-attestor`). The chain took **three** live-found links: **digest** (Jib's `jib-image.digest`), **runner KSA** (dind mode creates none → pin `serviceAccountName: arc-runner-scale-set`, create+annotate it in `04-githubactions.sh`; no `ignoreDifferences` — script-created), and **gcloud-on-PATH** (the `actions-runner` image ships no gcloud, unlike the other engines' `cloud-sdk`; added a `setup-gcloud` step, WI via the pod metadata). |
| **S9** | binauthz=on, `ci.engine` = **argoworkflows** | signing is per-engine | attestation by Argo | ✅ **live-validated 2026-07-17** (`gateway@sha256:e608…` + `jhipster@sha256:6b0f…`). Mirrors Tekton (ArgoCD-managed WorkflowTemplate + SA): `binauthz-enabled`/`project-id` params, `04-argoworkflows` annotates the `argoworkflows-ci` KSA, and `06-argoworkflows` patches the WorkflowTemplate param **defaults** so every submit path (seed runs, fallback, the Events Sensor) inherits signing; `ignoreDifferences` on the SA annotation + the two param values. Validating it surfaced an **orthogonal** engine-switch gap — the redeploy didn't provision the `argoworkflows-argocd` token, so `gitops-argocd-sync` 401'd (fixed: `scripts/08.55-argocd-ci-token.sh`, wired into the redeploys). **Binauthz signing now works on all four engines.** |
