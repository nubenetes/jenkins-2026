...
| **19** | **Day2** Update | **scale** | **02** Resume | [**`Day2.scale.02-resume`**](https://github.com/nubenetes/jenkins-2026/actions/workflows/Day2.scale.02-resume.yml) | Brings a paused cluster back: scales the node pools up and re-enables autoscaling (inputs default to `terraform/gke`'s node_count/min/max); pods reschedule and ArgoCD reconciles in minutes. | A paused cluster (row 18 run) | **To un-park** |
