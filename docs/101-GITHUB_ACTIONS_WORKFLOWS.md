...
| S8–S9 | binauthz=on, `ci.engine` = githubactions / argoworkflows | same, per-engine | attestation by that engine | ⬜ same pattern as S7, not yet run live. GHA render already threads the flag (`d3489d9`); Argo mirrors Tekton (ArgoCD-managed → needs the param + `ignoreDifferences`). Each needs the pipeline KSA annotated (for GHA the runner SA is `default`, not the `arc-runner-scale-set` terraform bound — resolve live) and an engine switch + build. |
