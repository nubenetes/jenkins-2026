...
| **Public endpoint** | this repo | `faro.<baseDomain>` HTTPRoute (no IAP) → `otel-collector-gateway:8027`, **TCP** HealthCheckPolicy (the receiver only answers POST). `config.yaml gateway.hosts.faro`, generated by `09-gateway.sh` |
...
The SDK lives on the gateway's **`develop` branch** (true branch-based promotion — feature on develop first), so: rebuild + deploy **gateway-develop** (the pipeline builds the develop branch — see [services.yaml](../jenkins/pipelines/seed/services.yaml)) → open the **develop** site in a real browser → beacons flow → the RUM dashboard (env=develop) fills. Promote the gateway `develop`→`main` to take RUM to the stable tier. **Without a browser**, the **`Day2.traffic.02` Synthetic RUM** workflow POSTs synthetic Faro beacons (incl. OTLP browser spans) to the endpoint to populate/validate the dashboard.
