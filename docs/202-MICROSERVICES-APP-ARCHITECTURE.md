...
- **Build-time source patch — MySQL → PostgreSQL + NoOp cache.** The upstream gateway is *generated for MySQL*, but this platform runs PostgreSQL (CloudNativePG); JHipster's default gateway also ships a **Hazelcast** 2nd-level cache whose Kubernetes pod-discovery needs RBAC. Rather than diverge the fork, the shared [`resources/patch-app-source.sh`](../resources/patch-app-source.sh) — run by **all four CI engines** right after checkout, before build — converts the gateway to **PostgreSQL** (JDBC/R2DBC drivers, Liquibase dialect, JDBC/R2DBC URLs, Testcontainers) **and** swaps the cache for a **NoOp cache** (a `CacheConfiguration` returning a `NoOpCacheManager`), so the cache code still wires up but against no real cache. See [402 § Shared app-source patch](./402-PIPELINES_AS_CODE.md#shared-app-source-patch-all-four-engines), [502](./502-MICROSERVICES_GITOPS.md) and [902 Troubleshooting](./902-TROUBLESHOOTING.md).
- **Observability per layer.** Java backends → **OTel Java agent** (traces/metrics/logs + JVM internals, see [303](./303-JVM-TUNING.md)); Angular → **browser RUM** (Grafana Faro / OTel JS), correlated per layer (**browser → gateway → microservice → DB**; true single-trace browser→backend correlation via W3C `tracecontext` is the next step — see below). The in-cluster OTel collector ingests the browser RUM beacon (faro receiver), and the Faro Web SDK **is wired into** the Angular client (gateway repo — next section). See [301](./301-OBSERVABILITY.md).

## Frontend observability — Angular RUM with Grafana Faro (implemented)

**Full-stack observability is now closed.** OTel covers the Java backends; the browser layer is covered by **Real User Monitoring (RUM)** via the **[Grafana Faro Web SDK](https://grafana.com/oss/faro/)** — chosen over the vendor-neutral OTel JS browser SDK (see the *Alternative considered* note below) for its turnkey Web-Vitals/errors/sessions/traces in one SDK — instrumented into the Angular SPA. It captures **Core Web Vitals** (LCP / INP / CLS / TTFB / FCP), JavaScript errors, user sessions and browser spans — the client-side counterpart to the [JVM internals](./303-JVM-TUNING.md) story for the backends.
...
**Status: live on `develop`, promoted to `main`.**

- **Shipped on `develop` first.** The Faro SDK landed on the gateway's **`develop` branch** (feature-on-develop-first); the develop tier is deployed with it — image **`gateway:develop-<build#>-<app-sha>`** (the immutable, rebuild-safe tag scheme) — and the **served Angular bundle includes the Faro SDK** pointing at the public endpoint (verified).
- **See it live.** Open the **develop** site (`microservices-develop.<baseDomain>`) in a real browser → beacons flow to the RUM dashboard (`env=develop`).
- **Promoted to `main`.** The instrumentation was promoted with the Faro `environment` flipped to **`stable`**, so rebuilding the stable **`gateway`** job lights up RUM on the stable tier too (`env=stable`, same endpoint).
- **No browser handy?** The **`Day2.traffic.02` Synthetic RUM** workflow POSTs synthetic Faro beacons (incl. OTLP browser spans).
- **Build note.** The faro/otel transitive `.d.ts` reference Node types (`global`/`path`) → the gateway tsconfig needs `skipLibCheck: true` (see [902](./902-TROUBLESHOOTING.md)).

> Alternative considered: **OpenTelemetry JS browser SDK** (`@opentelemetry/sdk-trace-web`) — vendor-neutral browser traces over OTLP-http, but you build the dashboards. Faro was chosen for the turnkey RUM. Next step for true single-trace correlation: propagate **W3C `tracecontext`** from the browser into the gateway so the browser span roots the backend trace.
