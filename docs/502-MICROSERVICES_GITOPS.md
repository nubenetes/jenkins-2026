...
>
> The pipeline's **GitOps Update** stage ([`vars/microservicesDeploy.groovy`](../vars/microservicesDeploy.groovy)) deploys by pushing **directly** to the env's gitops branch (the **deploy branch** for stable — `main` in prod, `develop` on a develop-launched validation cluster; always `develop` for the develop tier — see § *Branch model* below) in the **[`jenkins-2026-gitops-config`](https://github.com/nubenetes/jenkins-2026-gitops-config)** repo — it bumps the image tag in `helm/microservices/values-<env>.yaml` and ArgoCD reconciles it. So that repo's `main` branch must **accept the CI's direct push**: it is protected only against force-pushes/deletions, **not** with *require-a-pull-request*. Turning on require-PR there (the GitHub default for a "protected" branch) makes the PAT-authenticated push get rejected (an admin PAT does not bypass branch protection), so **every deploy's GitOps Update fails** with no commit ever landing and the image tags frozen.
>
...
To ship a new build, the CI engine (Jenkins by default; equally Tekton, GitHub Actions/ARC, or Argo Workflows — whichever `ci.engine` selects) does **not** `kubectl apply` anything — it edits one line (the image tag) with `yq`, `git push`es it to that repo's env branch (the deploy branch for stable — `main` in prod — and `develop` for the develop tier; § *Branch model*), and ArgoCD notices and rolls it out. Adding a whole new service is **one new row**. The only sharp edge is at **teardown**: deleting Services/Gateways leaves GCP load-balancer plumbing (NEGs) being cleaned up in the background, and tearing the cluster down too fast orphans them — so a "synchronization barrier" waits for them first.
...
- **CI writes the gitops branch directly.** The active CI engine's *GitOps Update* stage runs `yq -i '.services.<svc>.image.tag=…'` then `git push origin <branch>` — the **deploy branch** for the stable tier (`main` in prod; a develop-launched cluster pushes gitops `develop`, matching the AppSet's `{{branchStable}}` auto-track — § *Branch model*), `develop` for the develop tier — same contract across all four engines (Jenkins [`vars/microservicesDeploy.groovy`](../vars/microservicesDeploy.groovy), Tekton, GitHub Actions/ARC, Argo Workflows). That repo's `main` is therefore **direct-push** (force-push-blocked only) — *not* require-PR — the deliberate opposite of this infra repo's strict GitFlow. Require-PR there would reject the PAT push and wedge **every** stable deploy.
