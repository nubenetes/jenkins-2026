...
against the internal CA. Stages 1–6 convert **Headlamp**, the **faro RUM
receiver**, **ArgoCD** (the third only for the jenkins/githubactions CI
engines — see the roadmap), **pgAdmin**, the in-cluster **OSS Grafana**
(the fifth only in `observability.mode=oss` — doubly conditional), and
**Jenkins** (the sixth only for `ci.engine=jenkins`); the roadmap below has the
...
| **Jenkins** | chart `controller.httpsKeyStore.*` + cert-manager `keystores.jks` (JKS + password Secret) | ✅ **done (stage 6, `ci.engine=jenkins` only)** — highest blast radius of the six stages: build agents and (oss mode) Grafana's Jenkins datasource dial the controller **Service** directly in plain HTTP, so instead of requiring every in-cluster caller to trust the internal CA, [`values-backend-tls.yaml`](../helm/jenkins/values-backend-tls.yaml) uses the chart's **native** `controller.httpsKeyStore` feature: it moves the pod's plain-HTTP listener + kubelet probes to `httpsKeyStore.httpPort` (**8081**) while the Service's existing port (8080) becomes HTTPS, and `controller.extraPorts` re-exposes that plain pod port on the Service as **8082** (`port: 8082 → targetPort: 8081`). The Service port must differ from `8081`: the chart's `extraPorts` range emits *both* a Service port and a `containerPort` per entry, so reusing `8081` collides with the `http` `containerPort` `httpsKeyStore` already declares → an ArgoCD `ComparisonError` ("duplicate entries for key `containerPort=8081`") that silently fails the whole overlay. `08.7-backend-tls.sh` mints a JKS keystore (`keystore.jks`) instead of a plain PEM cert, encrypted with a password from a small dedicated `jenkins-https-jks-password` Secret (cert-manager reads an *existing* Secret via `passwordSecretRef`, it doesn't create one). `04-jenkins.sh` patches `JENKINS_AGENT_PORT=8082` (the Service port) into `jenkins-credentials` only when active, so [`jcasc-base.yaml`](../jenkins/casc/jcasc-base.yaml)'s Kubernetes-cloud `jenkinsUrl` (`${JENKINS_AGENT_PORT:-8080}`) follows — build agents keep dialing plain HTTP, just on the internal-only Service port (→ pod `8081`), with zero CA-trust plumbing in the agent JVM. **Known gap, deliberately not converted** (same shape as the ArgoCD/tekton gap above): in `observability.mode=oss`, Grafana's `grafana-jenkins-datasource` (`observability/grafana/values-oss.yaml`) still hardcodes `http://jenkins...:8080` for build-history panels — that file is fed to a third-party chart via ArgoCD's multi-source Helm rendering, not this repo's `sed`/`yq` templating, so making it conditional would mean reworking how that chart consumes values for one narrow (`oss` + `jenkins` + flag-on) case. The panel errors visibly in the Grafana UI rather than failing silently |
