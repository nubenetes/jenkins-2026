...
#### 3. Unsynced NEG Status Deadlock (Null Sync Time / No Zones)
* **The Problem**: During CI engine switches or redeployments, GKE's NEG controller cache can get wedged. The `ServiceNetworkEndpointGroup` (`svcneg`) resource gets created, but GKE fails to sync its status, leaving `status.lastSyncTime: null` and `status.networkEndpointGroups` empty. Because GKE Gateway depends on this status to translate HTTPRoutes, the Gateway controller is unable to find zones for the NEG. It fails the translation, wedging the Gateway's programmed status to `False` and breaking ingress routing for *all* hosts with `no zones found` errors.
* **The Self-Healing Fix**: We extended the proactive cleanup loop in `scripts/01-namespaces.sh` to automatically detect any `svcneg` resources with empty sync status. It waits a 15-second grace period. If they remain unsynced, it automatically self-heals them: it backs up the service's `cloud.google.com/neg` annotation, temporarily removes the annotation from the Service (which forces the GKE NEG controller to flush its cache), purges the stuck `svcneg` CR by stripping its finalizers, and then restores the NEG annotation. This triggers a fresh, clean sync, populating the zones and immediately restoring the Gateway to `PROGRAMMED: True`.
