...
| **attestor / attestation** | **Binary Authorization** primitives — an *attestation* is a signed statement that an image **digest** cleared the pipeline gates; the *attestor* binds a Cloud KMS public key + a Container Analysis note, and the cluster admission policy trusts it. | [507](./507-BINARY-AUTHORIZATION.md) |
| **Backstage** | The **developer portal / IDP** (`backstage.enabled`, default **on**): software catalog + the **active engine's** CI/CD tab (one custom image ships all four engines' plugins, switched at runtime) + ArgoCD/Kubernetes views + **TechDocs** (these docs in-portal), at `backstage.<domain>` behind IAP with in-app **JWT-verified** `gcpIap` sign-in. One-time app-image bootstrap via `Day2.publish.06`. | [505](./505-BACKSTAGE.md) |
| **Binary Authorization** | GKE-native **admission control** (`security.binaryAuthorization.enabled`, default off) that admits only images carrying a valid **attestation** signed by this project's attestor — so only scanned/signed images run (`dryrun` logs, `enforce` blocks). Orthogonal to the TLS/mesh axis. | [507](./507-BINARY-AUTHORIZATION.md) |
| **Cloud Service Mesh (CSM)** | Google's **managed Istio** service mesh (`serviceMesh.mode=cloud-service-mesh`, default `none`) — workload-identity mTLS + L7 authZ on every hop, via the **standalone à-la-carte SKU** (per mesh client ~$0.50/mo; **not** a GKE Enterprise tier — editions dissolved 2025-09). Mutually exclusive with backend TLS. | [506](./506-SERVICE-MESH.md) |
...
| **Dataplane V2** | GKE's **Google-managed** Cilium/eBPF data plane (`datapath_provider = ADVANCED_DATAPATH`) — the reason NetworkPolicies actually **enforce** (and what the WireGuard inter-node encryption rides on). Google-managed, so you can't layer upstream Cilium / Cilium Service Mesh on top. An immutable cluster field. | [503](./503-NETWORKING.md) |
...
| **Fleet** (GKE Hub) | A Google **fleet** groups clusters for fleet-scoped features; a `google_gke_hub_membership` registers this cluster (free in standard GKE since 2025-09). The persistent, fixed-identity membership **CSM** needs — rebuild-safe by *reconcile-to-current* ([104](./104-REBUILD_SAFETY.md)). | [506](./506-SERVICE-MESH.md) |
| **IAP** | **Identity-Aware Proxy** — Google's edge identity check in front of the admin UIs (Jenkins, Headlamp, pgAdmin, Backstage, Tekton/Argo dashboards) via `GCPBackendPolicy`. The public `microservices` host has no IAP. | [501](./501-PLATFORM_OPERATIONS.md) |
...
| **LiteLLM** | An OpenAI-compatible **LLM proxy** — here a stateless in-cluster pod that lets the Grafana LLM app reach **Vertex AI Gemini** keylessly over Workload Identity (`observability.llm.enabled`, oss mode only; ClusterIP-internal, nothing public). | [301 § Grafana LLM app](./301-OBSERVABILITY.md#grafana-llm-app-ai-assistant--opt-in-keyless-oss-only) |
| **LLM app (`grafana-llm-app`)** | The official Grafana plugin that centralizes LLM access for Grafana's AI features (`@grafana/llm`). **Not** the Grafana Cloud-only "Grafana Assistant". Enabled by `observability.llm.enabled` in oss mode; deliberately **not** wired to the managed Grafanas (no keyless path — see the decision note). | [301 § Grafana LLM app](./301-OBSERVABILITY.md#grafana-llm-app-ai-assistant--opt-in-keyless-oss-only) |
| **mTLS** | **Mutual TLS** — both ends present a certificate, so each cryptographically proves its identity (vs one-way TLS, where only the server does). A **service mesh** gives *workload-identity* mTLS on every hop; backend TLS is one-way, one hop. | [506](./506-SERVICE-MESH.md) |
...
| **service mesh** | A dedicated infra layer (control plane + per-pod/-node proxies) adding **mutual-identity mTLS + L7 authorization** between workloads on every hop. Here provided only by **Cloud Service Mesh**, opt-in and off by default (`serviceMesh.mode`). | [506](./506-SERVICE-MESH.md) |
| **SPIFFE** | **Secure Production Identity Framework For Everyone** — the open standard for cryptographic *workload* identity (a `spiffe://…` URI baked into each proxy's cert) that a mesh's mTLS uses to name services. | [506](./506-SERVICE-MESH.md) |
...
| **default-NAP overflow node** | An **untainted**, *smallest-that-fits* node (e.g. `e2-medium`, 940m CPU) GKE's cluster-level NAP auto-provisions when a **platform** pod briefly can't fit the static pool — distinct from a `ci-spot` **Spot** node (floored at 4 vCPU) and the **only** path that yields tiny nodes. Too small for the standard DaemonSets, so "chase-every-node" DaemonSets (image prepull, node-exporter) are `nodeAffinity`-scoped off it. | [501](./501-PLATFORM_OPERATIONS.md#scoping-the-standard-daemonsets-off-the-tiny-default-nap-overflow-nodes) |
