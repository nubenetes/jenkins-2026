...
| Deployments tab shows no data (works in `argocd` UI) | the plugin backend calls the configured instance `url` — if it points at the IAP-protected public host, every API read 302s to Google sign-in; if internal, the `argocd-baseline` NetworkPolicy must allowlist the `backstage` namespace (it does) | keep `instances[].url` INTERNAL (`${ARGOCD_BASE_URL}`). Known upstream gap (plugin 2.9.0): deep links derive from the same instance url (`argocd.baseUrl` is only a dead fallback today), so "open in ArgoCD" links don't resolve from a browser — use `https://argocd.<domain>` directly until upstream splits the two |
