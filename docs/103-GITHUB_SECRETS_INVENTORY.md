...
| `GRAFANA_CLOUD_ASSISTANT_BACKEND_URL` | secret | optional (only if `observability.assistant.enabled`) | Grafana Assistant → your Cloud stack backend URL (oss chat) — [docs/301](./301-OBSERVABILITY.md) |
| `GRAFANA_CLOUD_ASSISTANT_INSTANCE_ID` | secret | optional (only if `observability.assistant.enabled`) | Grafana Assistant → your Cloud stack instance ID |
| `GRAFANA_CLOUD_ASSISTANT_TOKEN` | secret | optional (only if `observability.assistant.enabled`) | Grafana Assistant → the **raw** Grafana Cloud service-account token (`08.9` combines it as `instanceId:token`) |
...
**`GRAFANA_CLOUD_ASSISTANT_*`** — the Grafana Assistant chat connection (oss)

*Optional, and only when `observability.assistant.enabled=true` (oss). **Absent is a valid state** — the plugin installs but stays in the clean "pending" state, and the Day1 workflow does **not** fail (a missing GitHub secret resolves to an empty string, not an error). So you do **not** need dummy values. See [301 § Grafana Assistant](./301-OBSERVABILITY.md) for the cost model (data stays in GKE; only the free-tier Assistant AI uses Grafana Cloud).*

All three come from **your own Grafana Cloud stack** — any free-tier account, independent of the Terraform-managed `grafana-cloud`-mode stack (which `oss` doesn't provision):

- `GRAFANA_CLOUD_ASSISTANT_BACKEND_URL` **+** `GRAFANA_CLOUD_ASSISTANT_INSTANCE_ID` — in Grafana Cloud, open the Assistant plugin → **Connection → Manual configuration**; copy the *Backend URL* and the numeric *Instance ID*.
- `GRAFANA_CLOUD_ASSISTANT_TOKEN` — the **raw** service-account token (do **not** pre-combine it; `08.9-grafana-assistant.sh` prepends `instanceId:`): Grafana Cloud → *Administration → Users and access → Service accounts → Add service account → Add token*.

**Create them.** The URL and instance ID are not sensitive (`--body` inline); the token is entered **interactively** so it isn't echoed or left in shell history. The commands are identical in `bash` and Windows PowerShell (`gh` is cross-platform; no `<` redirection here):

```bash
# bash — or Windows PowerShell (same commands)
gh secret set GRAFANA_CLOUD_ASSISTANT_BACKEND_URL --repo nubenetes/jenkins-2026 --body "https://<your-stack>.grafana.net"
gh secret set GRAFANA_CLOUD_ASSISTANT_INSTANCE_ID --repo nubenetes/jenkins-2026 --body "<numeric-instance-id>"
gh secret set GRAFANA_CLOUD_ASSISTANT_TOKEN       --repo nubenetes/jenkins-2026    # prompts: paste the raw token (not echoed)
```

Then re-run **`Day1.cluster.01` with `assistant_enabled=true`** — `08.9` builds the durable `grafana-assistant-credentials` Secret and Grafana loads the connection. Remove them later with `gh secret delete GRAFANA_CLOUD_ASSISTANT_TOKEN --repo nubenetes/jenkins-2026` (and the other two).

> **PowerShell note:** these three take plain string values, so `--body "…"` and the interactive prompt behave exactly as in bash — *unlike* the ARC private key ([§ 9.5](#registering-the-arc-github-app--step-by-step)), which needs `Get-Content -Raw … | gh secret set …` because PowerShell has no `<` operator.
