...
Used by `Day2.publish.05-publish-grafana-alerts` and `scripts/07.5-grafana-alerts.sh`. The script resolves the contact-point email with this priority chain (highest → lowest):
...
Used by `Day0.infra.03-azure-bootstrap`, `Day1.cluster.01-gke-provision`, `Day2.publish.03-publish-azure-dashboards`, `Day2.publish.05-publish-grafana-alerts`, `Decom.infra.03-azure-decommission`. These are **identifiers only** — no secret credential values. The actual Azure credentials are obtained at runtime via GitHub OIDC federation; no client secret is ever stored.
...
Used by `Day0.infra.04-aws-bootstrap`, `Day1.cluster.01-gke-provision`, `Day2.publish.04-publish-aws-dashboards`, `Day2.publish.05-publish-grafana-alerts`, `Decom.infra.04-aws-decommission`. All three are **identifiers only** — authentication is via OIDC `AssumeRoleWithWebIdentity`; no access keys stored.
...
Used by `Day1.cluster.01-gke-provision` and `Day2.deploy.04-redeploy-headlamp`. Required only if the Gateway / IAP feature is enabled (`config/config.yaml → gateway.baseDomain`).
...
Used by `Day1.cluster.01-gke-provision` and `Day2.deploy.04-redeploy-headlamp`. Needed only if microservice images are pulled from a private registry or the Microservices source repo is private.
