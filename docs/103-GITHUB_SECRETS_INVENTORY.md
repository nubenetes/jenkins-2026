...
| `GRAFANA_CLOUD_ASSISTANT_TOKEN` | secret | optional (only if `observability.assistant.enabled`) | Grafana Assistant → a Grafana Cloud service-account token (combined as `instanceId:token`) |
