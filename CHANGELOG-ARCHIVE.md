...
Archived release history for **jenkins-2026**, versions **v0.1.0 – v0.28.56**.
The current changelog (`Unreleased` + the latest releases) lives in
[`CHANGELOG.md`](CHANGELOG.md); the versioning + release-cut convention is in
[`RELEASING.md`](RELEASING.md). Entries are preserved verbatim; a few were
**backfilled** (retroactively documented from the git history — see the PR
citations) during the v0.29.0 consolidation to make this a complete record.
...
### Fixed
- **Azure & AWS Managed Grafana showed the OLD, pre-optimization dashboards.** The
  `dashboards-azure/*-azure.json` and `dashboards-aws/*-aws.json` variants are pre-generated by
  `generate.py` and committed, but were not regenerated when the canonical dashboards were refreshed
  with the AI-optimized versions (v0.28.22) — so managed-azure/managed-aws published stale boards
  (e.g. jenkins 13 vs 41 panels, postgres 11 vs 63). Regenerated both variant sets from the optimized
  canonical (Loki→Azure Monitor/CloudWatch, Tempo→Azure Monitor/X-Ray; Prometheus panels unchanged);
  panel counts now match the canonical 1:1 (41/63/48/39/34/27). **OSS needs nothing** — it renders the
  canonical `observability/grafana/dashboards/` directly via the `oss-grafana-dashboards` ArgoCD app,
  and the canonical's `${DS_PROMETHEUS}` var + `loki`/`tempo` uids match the OSS-provisioned datasources.
...
### Fixed
- Self-heal the otel-collector-gateway rollout on an observability-mode switch — pre-delete the gateway Deployment (and stale ConfigMap) before each helm upgrade so the immutable-selector/PVC conflict no longer leaves the collector stuck or crash-looping (#217).
...
### Added
- GKE-provision workflow now auto-bootstraps the matching cloud observability backend (grafana-cloud/managed-azure/managed-aws) as an idempotent pre-flight job via `workflow_call`, so switching modes no longer leaves the backend Terraform outputs unapplied (#225, #227).
...
### Changed
- grafana-cloud stack is now ephemeral with a Terraform-generated slug (`<prefix><random>`, an output read from GCS state) instead of a fixed `GRAFANA_CLOUD_STACK_SLUG`, removing three GitHub secrets/vars and avoiding reserved-slug collisions on re-provision; the token module now does a two-phase (targeted-then-full) apply so the `grafana.instance` provider can configure on a fresh stack (#236, #238).
...
### Changed
- Upgraded both k6 entry points to 2.0.0 (OTel exporter flag `experimental-opentelemetry` → `opentelemetry`) and realigned the shared k6 dashboards to the k6 2.x condition-labelled `Rate` metric schema (#242).
...
### Fixed
- Microservices "5xx Error Rate by Service" and "HTTP Error Rate %" panels now read 0 (zeroed-series fallback) instead of "No data" when there is no 5xx traffic (#234).
- Empty Grafana custom panels across all modes — pin the Prometheus datasource `timeInterval=60s` so `rate()` panels populate, enable AMG `pluginAdmin` so the X-Ray datasource installs, and correct k6/Jenkins metric labels in the canonical dashboards (#232).
- managed-aws Grafana alert provisioning read the wrong secret keys (`AWS_AMG_ENDPOINT`/`AWS_AMG_WORKSPACE_ID` vs. the stored `GRAFANA_BASE_URL`/`GRAFANA_WORKSPACE_ID`), erroring on every deploy; also skip cleanly when no AWS credentials are present (#229).
...
### Added
- Terraform-managed AMG Grafana Admin assignment for AWS IAM Identity Center users (`aws_grafana_role_association`, gated on the new `AWS_GRAFANA_ADMIN_SSO_EMAILS` secret) so admin access is granted at bootstrap with no manual console steps (#230, #231).
...
### Fixed
- Dynamically populate `GRAFANA_BASE_URL` for Jenkins (from the `grafana-cloud-credentials` Secret, or hardcoded in oss mode) and patch it into the `jenkins-credentials` Secret so the dashboard link resolves on a fresh cluster (#50).
